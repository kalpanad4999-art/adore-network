...
const AuthorizationGate = ({ children }: { children: ReactNode }) => {
  const { authorized, roleError, loading, refresh } = useStudio();
      ...
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <CardTitle className="font-display text-2xl">
            {roleError ? "We couldn't verify your account" : "You are not authorized"}
          </CardTitle>
          <CardDescription>
            {roleError ? (
              <>Your workspace access couldn't be loaded just now. Check your connection and try again.</>
            ) : (
              <>
        ...
        <CardContent className="space-y-2">
          <Button
            variant="outline"
            className="w-full"
            onClick={async () => { await refresh(); navigate("/", { replace: true }); }}
          >
            Try again
          </Button>
